        ...
        <div class="my-careerlink-screen">
            <div class="my-careerlink bg-white mb-md-4 mb-0">
                <div class="container px-0 px-lg-3">
                    <h1 class="h4 page-title p-3 px-md-0 m-0">Thư xin việc đã nhận</h1>
                </div>
            </div>
            <div class="filters">
                ...
                <select id="job-title" onchange="redirectToServlet()">
                    <c:forEach items="${jobs}" var="j">
                        <option value="${j.id}" <c:if test="${j.id == selectedJobId}">selected</c:if>>${j.title}</option>
                    </c:forEach>
                </select>
                ...
                <div class="table-container">
                    <table>
                        ...
                        <c:forEach items="${files}" var="f">
                            <tr>
                                <td class="has-data">
                                    <a href="download-file?path=${f.encodingPath}" class="text-truncate">${f.title}</a>
                                </td>
                                <td class="has-data">${f.convertCreated}</td>
                                <td class="has-data">${f.status}</td>
                                <td class="has-data">${f.type}</td>
                                <td class="has-data">${f.title}</td>
                                <td class="has-data">${f.title}</td>
                            </tr>
                        </c:forEach>
                        </tbody>
