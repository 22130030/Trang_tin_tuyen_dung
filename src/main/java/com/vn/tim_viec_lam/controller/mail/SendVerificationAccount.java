...
@WebServlet(name = "send-verification-account",value = "/send-verification-account")
public class SendVerificationAccount extends HttpServlet {
    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (userService.isEmailExists(email)) {
            ...
            String resetLink = "http://localhost:8080/trang_tin_tuyen_dung/verification-account?token=" + token + "&key=" + tokenId;


            // Nội dung email
            String header = "Xin chào " + email + ",<br><br>";
            String content = "Bạn đã xác thực tài khoản. Nhấn vào liên kết sau để tiếp tục:<br><br>";
            ...
            resp.sendRedirect("login_reset_request.jsp?status=success");
        } else {
            resp.sendRedirect("login_reset_request.jsp?status=notfound");
        }
    }
}
